// Use the Kruskal method to build the smallest tree
// A small square in the maze is used as a cell, and the four sides of the cell are called walls
// Step1: Put all the walls into an array and sort them randomly
// Step2: Build and check all the cells. Initially, each cell is a separate set, with itself as the parent root node
// Step3: Visit the wall array in order and select a wall
// Step4: If the two cells connected by the wall belong to the same set, then no processing is done; otherwise, tear down the wall and merge the sets where the two cells are located
